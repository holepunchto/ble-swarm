...
module.exports = class BLETransport extends ReadyResource {
  ...
  // Retune to another topic live: only the advertised/scanned uuid changes, the
  // hosted service is untouched. Existing links are dropped — a switch tunes
  // away from the old topic's peers.
  async setTopic(topic) {
    const next = toServiceUUID(topic, this.tag)
    if (next === this.discoveryUUID) return
