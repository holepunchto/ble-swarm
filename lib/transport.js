...
// a pending server session must see its pipe (channel sid or first GATT data)
...
module.exports = class BLETransport extends ReadyResource {
  constructor({
    ...
    /** data pipes by name — _pipeFor picks one per session */
  ...
  // what we offer in OPEN/HELLO flags
  _capabilities() {
    return this.pipe === 'l2cap' ? PIPE_L2CAP : 0
  }

  // The one place that decides a session's pipe, from what both sides offered.
  // Returns null when no pipe suits both — l2cap refuses gatt-only peers
  // entirely so links never silently ride the wrong pipe.
  ...
  _onServerFrame(data) {
    ...
    if (f.type === TYPE_OPEN) {
      ...
      if (name === 'l2cap') {
        ...
        this._gatt.notify(encodeFrame(TYPE_HELLO, sid, hello)).catch(safetyCatch)
      } else {
        this._gatt.openServer(f.sidHex, session)
        this._gatt.notify(encodeFrame(TYPE_HELLO, sid, this.keyPair.publicKey)).catch(safetyCatch)
      }
      ...
      if (!s.stream) {
        ...
        this._log('server refused gatt data on l2cap session', f.sidHex)
  ...
  // A pipe hands its finished stream here — session wiring, dial state,
  // peripheral refs and close-time cleanup are transport concerns shared by
  // every pipe, so the pipes stay pure byte movers.
  _bindServerStream(sidHex, session, stream, name) {
    if (session.pipeTimer) clearTimeout(session.pipeTimer)
    session.pipeTimer = null
    session.stream = stream
    stream.on('close', () => this._closeServerSession(sidHex, session.sid))
    session.conn = this._onChannel(stream, false, null)
    this._log(`server pipe: ${name}`, sidHex)
  }

  _bindCentralStream(peripheral, sess, stream, name) {
    stream.on('close', () => {
      this._gatt.send(peripheral, encodeFrame(TYPE_CLOSE, sess.sid), true).catch(safetyCatch)
  ...
  _startCentralSession(peripheral, char) {
    ...
    this._gatt.send(peripheral, encodeFrame(TYPE_OPEN, sid, open), true).catch(safetyCatch)
    try {
  ...
  _onCentralNotify(peripheral, data) {
    ...
    if (f.type === TYPE_HELLO) {
      ...
      Promise.resolve(this._pipes[name].openCentral(peripheral, sess, { flags, psm })).catch(
        safetyCatch
      )
    } else if (f.type === TYPE_DATA) {
  ...
  async _sayGoodbye() {
    ...
    for (const { sid } of this._sessions.values()) {
      sent.push(this._gatt.notify(encodeFrame(TYPE_CLOSE, sid)).catch(safetyCatch))
    }
    for (const d of this._devices.values()) {
      const peripheral = d.peripheral
      const sess = peripheral && peripheral._session
      if (!sess || !peripheral._char) continue
      const f = encodeFrame(TYPE_CLOSE, sess.sid)
