...
// The hosted data service sits on a fixed per-tag uuid so a topic switch never
// has to touch the GATT db (there is no removeService); the topic scopes
// discovery through the advertised/scanned uuid alone.
const DISCOVERY = b4a.alloc(32)

// Derive a stable 128-bit BLE service UUID from a topic. Only devices that
// compute the same UUID ever discover each other. No topic means the
// tag-derived default: the fixed DISCOVERY buffer, never a hole in the hash
// input (sodium under Bare aborts natively on undefined).
function toServiceUUID(topic, tag = 'ble-swarm') {
  const h = hash([b4a.from(tag), topic || DISCOVERY])
  const hex = b4a.toHex(h.subarray(0, 16))
...
module.exports = class BLETransport extends ReadyResource {
  constructor({
    ...
    this.discoveryUUID = toServiceUUID(topic, tag)
  ...
  // in-flight outbound dials (hyperswarm parity)
  get connecting() {
    let n = 0
    for (const d of this._devices.values()) if (d.timer) n++
    return n
  }

  // not accepting new work: torn down or radio paused
  get _halted() {
    return this._suspended || this.closing || this.closed
  }
  ...
  // Retune the advertised/scanned uuid; the hosted service is untouched. The
  // owner bounces the radio when live — lifecycle intent lives up there.
  setTopic(topic) {
    const next = toServiceUUID(topic, this.tag)
    if (next === this.discoveryUUID) return false
    this.discoveryUUID = next
    return true
  }
  ...
  _maybeAdvertise() {
    ...
    this._log('advertising', this.discoveryUUID)
    this.server.startAdvertising({ serviceUUIDs: [this.discoveryUUID] })
  }
  ...
  _notifyHello(id, payload) {
    return this._gatt.notify(encodeFrame(TYPE_HELLO, id, payload)).catch(safetyCatch)
  }

  _sendClose(peripheral, id) {
    return this._gatt.send(peripheral, encodeFrame(TYPE_CLOSE, id), true).catch(safetyCatch)
  }

  _sendOpen(peripheral, id, payload) {
    return this._gatt.send(peripheral, encodeFrame(TYPE_OPEN, id, payload), true).catch(safetyCatch)
  }
  ...
  _bindCentralStream(peripheral, sess, stream, name) {
    stream.on('close', () => {
      this._sendClose(peripheral, sess.id)
  ...
  _startScan() {
    ...
    this.central.startScan([this.discoveryUUID], this.scanOptions)
  ...
  _armScanRestart() {
    ...
    const hunting = this.peers.size === 0 && Date.now() < this._huntUntil
    const duty = this.peers.size > 0 || (this._online && !hunting)
    const dark = this.peers.size > 0 ? SCAN_DUTY_OFF : SCAN_DUTY_OFF_ONLINE
    ...
    this._scanTimer = setTimeout(() => {
      ...
      const stillHunting = this.peers.size === 0 && Date.now() < this._huntUntil
      if (this.peers.size > 0 || (this._online && !stillHunting)) {
        if (this._scanning) {
          this._stopScan()
          this._armScanRestart()
        } else {
  ...
  _scheduleSelfHeal() {
    ...
    if (this._healing || this._halted) return
    this._radioWasDown = false
    this._healing = true
  ...
  _onDiscover(peripheral) {
    ...
    if (this.peers.size >= this.maxOutbound) return // gossip covers the rest
  ...
  _startCentralSession(peripheral, char) {
    const id = b4a.toHex(randomBytes(SID_LEN))
  ...
  async _onCentralNotify(peripheral, data) {
    ...
    if (f.type === TYPE_HELLO) {
      ...
      if (!this._weInitiate(key)) {
        ...
        // lives.
        this._device(peripheral.id).peerKey = b4a.toHex(key)
  ...
  _endCentral(peripheral, sess, { keep = false } = {}) {
    ...
    if (sess) this._sendClose(peripheral, sess.id)
    peripheral._session = null
  ...
  _abortDial(peripheral, reason) {
    ...
    if (id) {
      ...
      d.failures += 1
      const base = this.peers.size === 0 ? DIAL_COOLDOWN_BASE_LONELY : DIAL_COOLDOWN_BASE
      d.coolUntil =
  ...
  _track(conn, peripheralId, isInitiator) {
    if (peripheralId !== null && conn.remotePublicKey) {
      this._device(peripheralId).peerKey = b4a.toHex(conn.remotePublicKey)
    }
    if (this.closing || this.closed) return
    const key = b4a.toHex(conn.remotePublicKey)
  ...
  _untrack(conn) {
    ...
    if (!this.closing && !this.closed) {
      ...
      if (this.peers.size === 0 && !this._suspended) {
