...
module.exports = class L2CapPipe {
  ...
  acceptServer(channel) {
    this.transport._log('server l2cap channel open, awaiting sid')
    readSidPreamble(
      channel,
      (sid, leftover) => {
        const sidHex = sid !== null ? b4a.toString(sid, 'hex') : null
        const session = sidHex !== null ? this.transport._sessions.get(sidHex) : undefined
        if (!session || session.stream) {
          this.transport._log('server l2cap channel unmatched', sidHex ?? 'timeout')
          try {
            channel.destroy()
          } catch (err) {
            safetyCatch(err)
        ...
        this.transport._bindServerStream(sidHex, session, stream, 'l2cap')
        if (leftover.byteLength) stream.receive(leftover)
      },
      this._timeout
    )
  }
