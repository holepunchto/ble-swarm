...
// Dumb byte duplex: fragments outbound writes, pushes inbound bytes.
...
module.exports = class GattPipe {
  ...
  openServer(session) {
    const stream = new GattStream({
      send: (payload) => this.notify(encodeFrame(TYPE_DATA, session.id, payload))
    })
    this.transport._bindServerStream(session, stream, 'gatt')
  }
  ...
  openCentral(peripheral, sess, hello) {
    const stream = new GattStream({
      ...
      send: (payload) => this.send(peripheral, encodeFrame(TYPE_DATA, sess.id, payload))
    })
  ...
  async _writeOnce(peripheral, frame, withResponse) {
    if (!withResponse) {
